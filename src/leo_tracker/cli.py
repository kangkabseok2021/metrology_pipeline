...
def main() -> None:
    ...
    gen_p.add_argument("--n-frames", type=int, default=10, help="Number of frames to generate")
    gen_p.add_argument("--output-dir", type=Path, default=Path("frames"), help="Output directory")
    ...
    proc_p.add_argument(
        "--input-dir", type=Path, default=Path("frames"), help="Input frame directory"
    )
    proc_p.add_argument(
        "--output", type=Path, default=Path("results.json"), help="Output JSON path"
    )
    proc_p.add_argument("--workers", type=int, default=None, help="Number of worker processes")
...
def _cmd_process(args: argparse.Namespace) -> None:
    ...
    print(f"Processing {len(frame_paths)} frames with {args.workers or 'auto'} workers…")
    proc = BatchProcessor()
    ...
    import datetime
    output_data = [
        json.loads(r.model_dump_json()) for r in batch.results
    ]
